...
// Wrapper for select elements to add a dropdown icon
...
const InputForm = ({ onGenerate, onReset, isLoading, openThankYouModal, ...props }) => {
  ...
  const hookOptions = { en: ["Problem Call Out", "Target Call Out", "Potential Benefit", "Specific Number", "Social Proof", "Active Voice", "Confidence & Conviction", "Pattern Interrupt", "Warning & Caution", "Negativity Bias"], id: ["Problem Call Out", "Target Call Out", "Potential Benefit", "Specific Number", "Social Proof", "Active Voice", "Confidence & Conviction", "Pattern Interrupt", "Warning & Caution", "Negativity Bias"] };
      ...
      <div className="p-6 bg-white/50 dark:bg-slate-800/50 rounded-2xl border border-gray-200/80 dark:border-slate-700/80 shadow-sm">
        <div className="space-y-4">
            <div>
                ...
                <div className="flex gap-2 rounded-lg bg-gray-200/80 dark:bg-slate-800/80 p-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <SelectWrapper>
                        <select id="style" className={inputStyle} value={languageStyle} onChange={(e) => setLanguageStyle(e.target.value)}>
                            {languageStyleOptions[language === 'id' ? 'id' : 'en'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                        </select>
                    </SelectWrapper>
                </div>
                 <div>
                    ...
                    <SelectWrapper>
                        <select id="audience" className={inputStyle} value={targetAudience} onChange={(e) => setTargetAudience(e.target.value)}>
                            {audienceOptions[language === 'id' ? 'id' : 'en'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                        </select>
                    </SelectWrapper>
                </div>
            </div>
             ...
             <div>
                ...
                <SelectWrapper>
                    <select id="hook-type" className={inputStyle} value={hookType} onChange={(e) => setHookType(e.target.value)}>
                        {hookOptions[language === 'id' ? 'id' : 'en'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                    </select>
                </SelectWrapper>
            </div>
            ...
            {contentType === 'single' ? (
                <div>
                    ...
                    <SelectWrapper>
                        <select id="count" className={inputStyle} value={scriptCount} onChange={(e) => setScriptCount(parseInt(e.target.value))}>
                 ...
                 <div>
                    ...
                    <SelectWrapper>
                        <select id="slide-count" className={inputStyle} value={carouselSlideCount} onChange={(e) => setCarouselSlideCount(parseInt(e.target.value))}>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex gap-2">
            ...
            {isLoading ? (
                ...
                <span>{uiText.generatingButton}</span>
                </>
            ) : (
                <>
                <Bot className="w-5 h-5" />
                <span>{uiText.generateButton}</span>
                </>
            )}
            </button>
            {generatedContent.length > 0 && !isLoading && (
            <button onClick={onReset} className="p-3 flex items-center justify-center bg-gray-200 hover:bg-gray-300 dark:bg-slate-700 dark:hover:bg-slate-600 text-gray-700 dark:text-gray-200 rounded-lg shadow-md transition-colors">
