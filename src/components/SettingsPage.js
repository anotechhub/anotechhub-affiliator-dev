...
const SettingsPage = ({
  systemPrompt, setSystemPrompt, savedSystemPrompt, onSaveSystemPrompt,
  apiMode, setApiMode, userApiKey, setUserApiKey, onSaveApiSettings,
  uiText, setCurrentPage
}) => {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="space-y-8">
        ...
        <div className="p-6 bg-white/50 dark:bg-slate-800/50 rounded-2xl border border-gray-200/80 dark:border-slate-700/80 shadow-sm">
          ...
          <button onClick={onSaveApiSettings} className="mt-4 inline-flex items-center justify-center gap-2 bg-custom-teal hover:bg-custom-teal-dark text-white font-bold py-2 px-4 rounded-lg shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all">
            {uiText.saveApiSettings}
          </button>
        </div>

        {/* System Prompt Card */}
        <div className="p-6 bg-white/50 dark:bg-slate-800/50 rounded-2xl border border-gray-200/80 dark:border-slate-700/80 shadow-sm">
          <h3 className="text-lg font-semibold mb-2">{uiText.systemPrompt}</h3>
          ...
          <button onClick={onSaveSystemPrompt} className="mt-4 inline-flex items-center justify-center gap-2 bg-custom-teal hover:bg-custom-teal-dark text-white font-bold py-2 px-4 rounded-lg shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all">
            {uiText.saveChanges}
          </button>
        </div>
        ...
        <div className="p-6 bg-white/50 dark:bg-slate-800/50 rounded-2xl border border-gray-200/80 dark:border-slate-700/80 shadow-sm">
          ...
          <div className="p-4 bg-gray-100 dark:bg-slate-900/50 rounded-lg">
            <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-wrap">{savedSystemPrompt}</p>
          </div>
        </div>
      </div>
    </div>
